defmodule CommentManager do
  ...
  def get_comment(comment_id, token) do
    case HTTPoison.get!("https://api.box.com/2.0/comments/#{comment_id}", Poison.encode!(%{Authentication: "Bearer #{token}"})) do
      %{body: body, status_code: 200} -> # In the event of a response code of 200 (Successful GET)...
        body
        |> Poison.decode!
        |> Map.get("message")
      %{status_code: status_code, body: body} -> # In the event of a non desired response...
        {:error, "Failed to get comment with ID: #{comment_id}. Received #{status_code}: #{body}"} # Provide more info about what happened.
    end
  end
